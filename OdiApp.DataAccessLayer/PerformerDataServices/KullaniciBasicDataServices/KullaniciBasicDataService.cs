...
public class KullaniciBasicDataService : IKullaniciBasicDataService
{
    ...
    public async Task<List<KullaniciBasic>> KullaniciListesiGetirByKayitGrubuKodu(string kayitGrubuKodu)
    {
        return await _dbContext.KullaniciBasic
           .Where(x => !string.IsNullOrEmpty(x.KayitGrubuKodu) && x.KayitGrubuKodu.Contains(kayitGrubuKodu))
           .ToListAsync();
    }
}